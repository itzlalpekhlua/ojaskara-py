...
def get_by_slug(slug: str):
    row = store.projects.find_one(slug=slug)
    if not row:
...
def create_project(body: ProjectBody, _user: dict = Depends(require_admin)):
    ...
    record = {
        ...
        "order": store.projects.next_order(),
        "published": True,
        "createdAt": now,
        "updatedAt": now,
        "images": [],
    }
    store.projects.insert(record)
    return record
...
def delete_project(project_id: str, _user: dict = Depends(require_admin)):
    removed = store.projects.delete(project_id)
    if removed:
...
def toggle_published(project_id: str, body: PublishBody, _user: dict = Depends(require_admin)):
    updated = store.projects.update(project_id, {"published": body.published})
    if not updated:
        return JSONResponse({"error": "Not found"}, status_code=404)
    return updated
...
def reorder(project_id: str, body: DirectionBody, _user: dict = Depends(require_admin)):
    store.projects.reorder(project_id, body.direction)
    return {"ok": True}
...
def add_gallery_image(project_id: str, body: AddImageBody, _user: dict = Depends(require_admin)):
    updated = add_image(store.projects, project_id, body.url, media_type=body.mediaType)
    if not updated:
        return JSONResponse({"error": "Not found"}, status_code=404)
    return updated
...
def delete_gallery_image(image_id: str, _user: dict = Depends(require_admin)):
    delete_image(store.projects, image_id)
    return {"ok": True}
...
def reorder_gallery_image(image_id: str, body: DirectionBody, _user: dict = Depends(require_admin)):
    reorder_image(store.projects, image_id, body.direction)
    return {"ok": True}
...
def set_featured_image(project_id: str, body: FeaturedImageBody, _user: dict = Depends(require_admin)):
    updated = set_featured(store.projects, project_id, body.url)
    if not updated:
        return JSONResponse({"error": "Not found"}, status_code=404)
    return updated
